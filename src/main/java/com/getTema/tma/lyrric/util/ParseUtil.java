package com.getTema.tma.lyrric.util;

import com.alibaba.fastjson.JSONArray;
import com.getTema.tma.lyrric.model.Area;
...
//        reqHeader = reqHeader.replaceAll("Host:", "");
    ...
    public  static List<TestVoList> getAge(){
        Map<Integer,String> map=new HashMap<>();
        String sx="鼠、牛、虎、兔、龙、蛇、马、羊、猴、鸡、狗、猪";
        List<String> sxList= Arrays.stream(sx.split("、")).collect(Collectors.toList());
        String jnsx="虎";// 改一下这个就可以。
        List<String> zqsxList=new ArrayList<>();
        for(String s:sxList){
            if(zqsxList.isEmpty() && !jnsx.equals(s)){
                   continue;
            }
            zqsxList.add(s);
        }
        ...
        List<TestVoList> list=new ArrayList<>();
         for(int i=1;i<=45;i++){
             map.put(i,zqsxList.get((i-1)%12));
             TestVoList testVoList=new TestVoList();
             testVoList.setAge(i);
             testVoList.setNum(0);
             testVoList.setSx(zqsxList.get((i-1)%12));
             list.add(testVoList);
         }
